...
- **Боковая панель (Навигация):**
  - Быстрый переход между разделами: Dashboard, Process Manager, Analytics, Players, Settings.
- **Process Manager (Раздел Управления):**
  - Централизованное управление сервером: Кнопки **Start**, **Stop**, **Restart**.
  - Конфигурация запуска: Путь к `.exe`, параметры запуска, авто-рестарт.
  - Статус процесса: Визуальная индикация (Running/Stopped).
- **Настройки:**
  - Параметры RCON подключения.
  - Настройка Планировщика (интервалы рестартов) и тексты уведомлений.
